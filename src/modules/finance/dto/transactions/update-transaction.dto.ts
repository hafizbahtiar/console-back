import { IsNumber, IsString, IsEnum, IsOptional, IsDateString, IsArray, Min, MaxLength, ArrayMaxSize, ValidateIf, IsNotEmpty, Matches } from 'class-validator';
...
export class UpdateTransactionDto {
    ...
    @IsOptional()
    @ValidateIf((o) => o.exchangeRate !== undefined && o.exchangeRate !== null)
    @Min(0, { message: 'Exchange rate must be greater than or equal to 0' })
    exchangeRate?: number; // Exchange rate at transaction time

    @IsNumber({}, { message: 'Base amount must be a valid number' })
    @IsOptional()
    @ValidateIf((o) => o.baseAmount !== undefined && o.baseAmount !== null)
    ...
    @IsNotEmpty({ message: 'Date must be a valid date string if provided' })
    date?: string;
